/// EIP-191 personal_sign authentication for community-agent.
///
        ...
        if stderr.contains("expired") || stderr.contains("invalid token") {
            bail!(
                "awp-wallet signing failed (possibly old token issue). Try without AWP_WALLET_TOKEN:\n\
                 unset AWP_WALLET_TOKEN && community-agent status\n\
                 Original error: {}",
                stderr.trim()
            );
        }
        ...
        if signer_lower != addr_lower {
            ...
            bail!(
                "Wallet address mismatch detected!\n\
                 ...
                 Try: unset AWP_AGENT_ID AWP_SESSION_ID && community-agent status",
                addr_lower,
                signer_lower
            );
        }
